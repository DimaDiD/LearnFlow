...
using LearnFlow.Enrollments.API.Middleware;
using LearnFlow.Enrollments.Application.Common.Behaviors;
using LearnFlow.Enrollments.Application.Features.Enrollments.Commands.EnrollStudent;
using LearnFlow.Enrollments.Infrastructure;
using LearnFlow.Enrollments.Infrastructure.Persistence;
using MediatR;
...
try
{
    Log.Information("Starting Enrollments Service...");
    ...
    builder.Host.UseSerilog((context, services, configuration) => configuration
        ...
        .Enrich.WithProperty("Service", "enrollments-service")
    ...
    builder.Services.AddValidatorsFromAssembly(typeof(EnrollStudentCommand).Assembly);
...
{
    Log.Fatal(ex, "Enrollments Service failed to start.");
}
finally
{
    Log.CloseAndFlush();
